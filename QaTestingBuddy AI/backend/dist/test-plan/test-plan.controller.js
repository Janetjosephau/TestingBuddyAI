"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.TestPlanController = void 0;
const common_1 = require("@nestjs/common");
const test_plan_service_1 = require("./test-plan.service");
const generate_test_plan_dto_1 = require("./dto/generate-test-plan.dto");
let TestPlanController = class TestPlanController {
    constructor(testPlanService) {
        this.testPlanService = testPlanService;
    }
    async generateTestPlan(generateTestPlanDto) {
        return this.testPlanService.generateTestPlan(generateTestPlanDto);
    }
    async getAllTestPlans() {
        return this.testPlanService.getAllTestPlans();
    }
    async getTestPlan(id) {
        return this.testPlanService.getTestPlan(id);
    }
};
exports.TestPlanController = TestPlanController;
__decorate([
    (0, common_1.Post)('generate'),
    __param(0, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [generate_test_plan_dto_1.GenerateTestPlanDto]),
    __metadata("design:returntype", Promise)
], TestPlanController.prototype, "generateTestPlan", null);
__decorate([
    (0, common_1.Get)(),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", []),
    __metadata("design:returntype", Promise)
], TestPlanController.prototype, "getAllTestPlans", null);
__decorate([
    (0, common_1.Get)(':id'),
    __param(0, (0, common_1.Param)('id')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String]),
    __metadata("design:returntype", Promise)
], TestPlanController.prototype, "getTestPlan", null);
exports.TestPlanController = TestPlanController = __decorate([
    (0, common_1.Controller)('test-plans'),
    __metadata("design:paramtypes", [test_plan_service_1.TestPlanService])
], TestPlanController);
//# sourceMappingURL=test-plan.controller.js.map